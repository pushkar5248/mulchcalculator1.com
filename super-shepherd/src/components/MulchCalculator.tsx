...
import { Plus, Trash2, Maximize2, ChevronDown, Layers, DollarSign, Calculator, RefreshCcw } from 'lucide-react';
...
const MulchCalculator: React.FC<Props> = ({ lang = defaultLang }) => {
  ...
  const handleClear = () => {
    setAreas([{ id: '1', shape: 'rectangle', name: 'Main Bed', length: 0, width: 0 }]);
    setDepth(0);
    setPricePerUnit(0);
    setBagSize(0);
    setPricePerBag(0);
        ...
        {!showResults ? (
          <div className="lg:col-span-3 max-w-4xl mx-auto w-full space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Areas Section */}
              <div className="lg:col-span-2 bg-canvas border border-hairline-strong rounded-sm overflow-hidden shadow-sm">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4 max-w-2xl mx-auto w-full">
              <button 
                ...
                className="classical-button bg-primary text-on-primary h-14 px-8 text-sm font-bold uppercase tracking-widest flex-1 flex items-center justify-center hover:scale-[1.02] active:scale-[0.98] transition-all shadow-lg"
              >
                <Calculator className="w-5 h-5 mr-2" /> {t('calc.calculate')}
              </button>
              <button 
                onClick={handleClear}
                className="classical-button h-14 px-8 text-sm font-bold uppercase tracking-widest flex-1 flex items-center justify-center border-hairline-strong hover:bg-canvas-soft transition-all"
              >
                <RefreshCcw className="w-5 h-5 mr-2" /> {t('calc.clear')}
              </button>
            </div>
          </div>
        ) : (
          <div className="lg:col-span-3 max-w-2xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-canvas border-2 border-primary rounded-sm p-8 shadow-md space-y-8">
              ...
              <div className="space-y-6">
                ...
                {totalVolume > 0 && (
                  <div className="pt-6 border-t border-hairline-strong">
                    <p className="body-sm text-body leading-relaxed italic">
                      {(() => {
                        const vCuMeters = unit === 'metric' ? totalVolume : totalVolume * 0.764555;
                        const vLiters = vCuMeters * 1000;
                        const vCuYards = unit === 'imperial' ? totalVolume : totalVolume * 1.30795;
                        const vCuFeet = vCuYards * 27;
                        const unitLabel = unit === 'imperial' ? t('calc.cuYard') : t('calc.cuMeter');
                        
                        return t('calc.conclusion')
                          .replace('{liters}', vLiters.toLocaleString(undefined, { maximumFractionDigits: 0 }))
                          .replace('{cuMeters}', vCuMeters.toLocaleString(undefined, { maximumFractionDigits: 2 }))
                          .replace('{cuYards}', vCuYards.toLocaleString(undefined, { maximumFractionDigits: 2 }))
                          .replace('{cuFeet}', vCuFeet.toLocaleString(undefined, { maximumFractionDigits: 1 }))
                          .replace('{price}', pricePerUnit.toFixed(2))
                          .replace('{unitLabel}', unitLabel)
                          .replace('{total}', totalCostBulk.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                      })()}
                    </p>
                  </div>
                )}
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button 
                ...
                className="classical-button h-14 px-8 text-sm font-bold uppercase tracking-widest flex-1 flex items-center justify-center border-hairline-strong hover:bg-canvas-soft transition-all"
              >
                <RefreshCcw className="w-5 h-5 mr-2" /> {t('calc.clear')} / Edit
              </button>
            </div>
          </div>
        )}
